-- 38 - SQL AVANÇADO - Criando índices na criação da tabela
/*
-- Criando índices para PRIMARY KEY  na criação da tabela
   > Se desejar utilizar o nome da CONSTRAINT para o nome do índice 
   (nome default)
   > Se desejar dar um nome específico para um índice criado com a CONSTRAINT de 
   PRIMARY KEY 
   > Você não pode remover um índice utilizado por uma CONSTRAINT de PRIMARY KEY
   ou UNIQUE;
   > Se desejar definir propriedades específicas para um índice criado com a 
   CONSTRAINT de PRIMARY KEY
*/

-- Criando índices para PRIMARY KEY  na criação da tabela com nome default
DROP TABLE projects;
